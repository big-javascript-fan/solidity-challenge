...
describe("when stake", function () {
  beforeEach(async () => {
    ...
    it("deposit fail when balance < amount or amount = 0 ", async () => {
      await expectRevert(
        stakerContract.connect(account1).deposit(ethers.utils.parseEther("300")),
        "Please deposit more in your card!"
      );
      await expectRevert(
        stakerContract.connect(account1).deposit(ethers.utils.parseEther("0")),
    ...
    it("deposit success", async () => {
      ...
      await stakerContract
        .connect(account1)
        .deposit(ethers.utils.parseEther("30"));
      expect(await stakerContract.depositOf(account1.address)).to.be.equal(
      ...
      // get the total amount of depositd tokens
      expect(await stakerContract.getTotaldepositdAmount()).to.be.equal(
        ethers.utils.parseEther("30")
      );
      // account2 deposit
      await rewardTokenContract
      ...
      expect(await stakerContract.depositOf(account2.address)).to.be.equal(
    ...
    describe("when withdraw", function () {
      describe("cases of withdraw fail", function () {
        ...
        it("withdraw fail when the amount of withdraw < the amount of stake", async () => {
          ...
          await expectRevert(
            ...
            "The amount to be transferred should be equal or less than deposit"
          );
        });
      });
      it("withdraw success", async () => {
        ...
        await stakerContract
          .connect(account1)
          .deposit(ethers.utils.parseEther("60"));
        await stakerContract
          .connect(account1)
        ...
        await stakerContract
          .connect(account2)
          .deposit(ethers.utils.parseEther("150"));
        await stakerContract
          .connect(account2)
  ...
  describe("when claim reward", function () {
    ...
    describe("check the calculation for it when the setting of reward rate success", async () => {
      ...
      it("check the total amount of the depositd tokens", async () => {
        expect(await stakerContract.getTotaldepositdAmount()).to.be.equal(
          ethers.utils.parseEther("300")
        );
      });
